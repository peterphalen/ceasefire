...
The results cohere well with ```prophet```, but this model fits the data better in many ways (e.g., we no longer see impossible predictions of less than zero shootings). Also, doing things with ```brms``` will allow us to analyze the model more carefully. Here's how we created the above plot:
