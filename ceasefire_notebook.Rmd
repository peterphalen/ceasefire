...
Here are the marginal seasonal and time trend effects, showing the components that make up the above time series. The specific numbers on the y-axis are dependent upon the reference point, but these figures gives you the right idea of the shape of the trends.
...
```{r, message=FALSE, warning=FALSE}
plot(m2, pars="ceasefireCeasefire Weekend") + scale_y_discrete(labels="ceasefire")
```

We can also use this model to see the impact of the ceasefire at specific points in time. For example, here is the effect of ceasefire on Saturdays during the summer:

```{r, echo=FALSE, message=FALSE, warning=FALSE}
p <- purrr::map( # use saturday as reference
       c("day.of.year [all]", "weekday [Saturday]", "julz [all]", "ceasefire"),
       ~ ggeffects::ggpredict(m2, .x, ci.lvl= .95) %>% plot() + theme_bw()
     )
...
p <- purrr::map( # index to Saturday for the ceasefire effect
       c("day.of.year [all]", "weekday [Saturday]", "julz [all]", "ceasefire"),
       ~ ggeffects::ggpredict(m2, .x, ci.lvl= .95) %>% plot() + theme_bw()
     )
